...
#average vot for each minimal pair

d.test.excluded %>%
  group_by(Item.MinimalPair) %>%
  summarise(mean_vot = mean(Item.VOT), .groups = "drop")



d.test.excluded$resp.num <- NA
d.test.excluded$resp.num[d.test.excluded$Response %in% c("dim", "dill", "dip")] <- 0
d.test.excluded$resp.num[d.test.excluded$Response %in% c("tim", "till", "tip")] <- 1
...
model <- glmer(resp.num ~ Item.VOT * Item.MinimalPair + (1 + Item.VOT.z | ParticipantID),
