...
d.test %<>%
  ...
  mutate(Response.log_RT = log10(ifelse(Response.RT <= 0, NA_real_, Response.RT)),
         ...
         ### Will: should be evaluating scale(Response.log_RT)?
         ### scale(Response.log_RT.mean) would give only the z-score of the subject mean (which is always 0)
         Excluded.participant.due.to.mean.RT = ifelse(abs(scale(Response.log_RT.mean)) > 3, TRUE, FALSE)) 
         ### hence no data was excluded (Excluded.participant.due.to.mean.RT has 0 observation)

         ### also, the codes here seem to intend to do trial-level RT exclusion within subjects -
         ### participant-level RT exclusion should be implemented before this through "summarise()"?
...
```{r will-check RT distribution}

hist(d.test.excluded$Response.RT, breaks = 50)
#very long RT tail!

table(d.test.excluded$Response.RT > 3000)
#451/3518 = 12.8%
...
d.test.excluded$resp.num <- NA
d.test.excluded$resp.num[d.test.excluded$Response %in% c("dim", "dill", "dip", "din")] <- 0
d.test.excluded$resp.num[d.test.excluded$Response %in% c("tim", "till", "tip", "tin")] <- 1
table(d.test.excluded$resp.num, useNA = "ifany")
#d.test.excluded$Response.num <- ifelse(d.test.excluded$Response == "right", 1, 0)

d.test.excluded %<>%
  mutate(Item.VOT.z = as.numeric(scale(Item.VOT)))

d.test.excluded$ParticipantID <- as.factor(d.test.excluded$ParticipantID)
d.test.excluded <- d.test.excluded |> 
  filter(ParticipantID != "23")

model <- glmer(
  resp.num ~ Item.VOT.z * Item.MinimalPair + (1 + Item.VOT.z | ParticipantID),
  data = d.test.excluded,
  family = binomial
)

m <- mean(d.test.excluded$Item.VOT)
sd <- sd(d.test.excluded$Item.VOT)

coef <- fixef(model)

calc_VOT_boundary <- function(intercept, slope, mean_raw, sd_raw) {
  intercept <- as.numeric(intercept)
  slope <- as.numeric(slope)
  z_boundary <- -intercept / slope
  raw_boundary <- z_boundary * sd_raw + mean_raw
  return(raw_boundary)
}

# reference level
boundary_dilltill <- calc_VOT_boundary(
  intercept = coef["(Intercept)"],
  slope     = coef["Item.VOT.z"],
  mean_raw  = m,
  sd_raw    = sd
)

# dimtim
boundary_dimtim <- calc_VOT_boundary(
  intercept = coef["(Intercept)"] + coef["Item.MinimalPairdimtim"],
  slope     = coef["Item.VOT.z"] + coef["Item.VOT.z:Item.MinimalPairdimtim"],
  mean_raw  = m,
  sd_raw    = sd
)

# diptip
boundary_diptip <- calc_VOT_boundary(
  intercept = coef["(Intercept)"] + coef["Item.MinimalPairdiptip"],
  slope     = coef["Item.VOT.z"] + coef["Item.VOT.z:Item.MinimalPairdiptip"],
  mean_raw  = m,
  sd_raw    = sd
)

boundary_dintin <- calc_VOT_boundary(
  intercept = coef["(Intercept)"] + coef["Item.MinimalPairdintin"],
  slope     = coef["Item.VOT.z"] + coef["Item.VOT.z:Item.MinimalPairdintin"],
  mean_raw  = m,
  sd_raw    = sd
)

ref_level <- levels(d.test.excluded$Item.MinimalPair)[1]
cat("Estimated VOT boundary for dilltill:", round(boundary_dilltill, 2), "ms\n")
cat("Estimated VOT boundary for dimtim:", round(boundary_dimtim, 2), "ms\n")
cat("Estimated VOT boundary for diptip:", round(boundary_diptip, 2), "ms\n")
cat("Estimated VOT boundary for dintin:", round(boundary_dintin, 2), "ms\n")

```

```{r a quick check on individual curves}

ggplot(d.test.excluded, aes(x = Item.VOT, y = resp.num)) +
...
#ggplot(d.test.excluded, aes(x = Item.VOT, y = resp.num, color = Item.MinimalPair)) +
#  geom_point() +
#  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE) 

#Will: use ggpredict instead to take random effects into account
full_vot_ms = seq(-100, 130, by = 0.5)
full_vot_z = (full_vot_ms - m) / sd

full_pred_fixed <- ggpredict(model, 
                       terms = c(paste0("Item.VOT.z [", paste(full_vot_z, collapse=","), "]"), "Item.MinimalPair"), 
                       type = "fixed")
plot_df_fixed <- as.data.frame(full_pred_fixed)

plot_df_fixed <- plot_df_fixed %>%
  mutate(VOT_ms = (x * sd) + m)

plot_df_fixed$group <- factor(
  plot_df_fixed$group, 
  levels = c("dilltill", "dintin", "diptip", "dimtim")
)
...
```
```{r group plot overlaid with individual curves}

pred_random <- ggpredict(model, terms = c(paste0("Item.VOT.z [", paste(full_vot_z, collapse=","), "]"), "Item.MinimalPair", "ParticipantID"), type = "random")
...
d.IOs <- 
  rbind(talker.VOT.no.noise, talker.VOT, talker.VOT_F0, talker.VOT_F0.centered) %>% 
  select(Talker, gender, io, io.type)

# We need to give the IOs that are built on 2 cues, the F0 values of the stimuli. T
d.f0.5ms <- 
  read_csv("../data/AEDLVOT_stimuli_f0_5ms.csv") %>% 
...
d.temp <- 
  ...
  # this incorporates the F0 measurements that align better with C&W
  mutate(
    Item.F0.5ms = normMel(f0_5ms),
...
d.temp %<>%
  mutate(
    likelihood_per_response =
      pmap(
        list(d.perception, io, io.type), 
        ~ get_average_accuracy_of_IO(
          observations = if (str_detect(..3, "VOT_F0")) ..1$VOT_F0 else ..1$Item.VOT, 
...
d.temp %>%
  group_by(io.type) %>%
  summarise(
    # Here you could use bootstrap to get the lower and uppper CI within each group
    mean_likelihood_per_response = mean(likelihood_per_response) 
  )
```
