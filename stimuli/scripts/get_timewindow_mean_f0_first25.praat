...
header$ = "Filename	TextGridLabel	duration	f0_25%_mean	F1_25%_mean	F2_25%_mean'newline$'"
	...
	if fileReadable (gridfile$)
		...
		for interval to numberOfIntervals
			...
			if label$ <> ""
				...
				t_start = start
				t_end = start + 0.25 * duration

				# formants:
				select Formant 'soundname$'
				#f1_25 = Get value at time... 1 midpoint Hertz Linear
				#f2_25 = Get value at time... 2 midpoint Hertz Linear

				# pitch:
				select Pitch 'soundname$'
				f0_25 = Get mean: t_start, t_end, "Hertz"

				# Save result to text file:
				resultline$ = "'soundname$'	'label$'	'duration'	'f0_25'	'f1_25'	'f2_25' 'newline$'"
